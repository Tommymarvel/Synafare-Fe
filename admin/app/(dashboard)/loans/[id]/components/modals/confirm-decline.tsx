...
const ConfirmDecline = ({
  ...
  onConfirm: () => Promise<boolean>; // parent will do the API and return success/failure
  submitting?: boolean;
}) => {
  const handleConfirm = async () => {
    const ok = await onConfirm();
    if (ok) onOpenChange(false); // close confirm on success
    // if not ok, stay open so user can retry or cancel
  };
