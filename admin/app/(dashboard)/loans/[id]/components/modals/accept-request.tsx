...
export default function AcceptRequestModal({
  ...
  const monthlyRatePercentage = defaultMonthlyRate * 100;
  const [showConfirmAccept, setShowConfirmAccept] = useState(false);

  // form state
  const [amountOffered, setAmountOffered] = useState<number>(amountRequested);
  const [monthlyRate, setMonthlyRate] = useState<number>(monthlyRatePercentage);
  ...
  useEffect(() => {
    if (open) {
      ...
      setMonthlyRate(monthlyRatePercentage);
      setDurationMonths(defaultDurationMonths);
      setErrorMsg('');
    }
  }, [open, amountRequested, monthlyRatePercentage, defaultDurationMonths]);
  ...
  const computed = useMemo(() => {
    ...
    const totalInterest = amountOffered * (monthlyRate/100) * durationMonths;
  ...
  const disabled =
   ...
   async function handleSendOffer(): Promise<boolean> {
    setErrorMsg('');
