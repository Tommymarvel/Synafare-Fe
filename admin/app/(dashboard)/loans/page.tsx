...
import { filterLoans } from '@/lib/filterLoans';
...
const AllLoans = () => {
  ...
  const requested = allLoans.reduce(
    (acc, loan) => acc + (loan.loanAmount ?? 0),
    0
  );
  const disbursed = allLoans.reduce(
    (acc, loan) =>
      acc + (loan.repayments.reduce((sum, r) => sum + (r.amount ?? 0), 0) ?? 0),
    0
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-4">
        <div className="rounded-[6px] border border-gray-4 w-fit min-w-[369px] text-resin-black py-6 px-4 flex items-center justify-between">
          {/* left: title + two figures */}
          <div className="space-y-3">
            <h3 className="text-[13px] font-medium text-[#D4A017]">
              Loan Amount
            </h3>
          ...
          <div
            className={cn(
              'w-10 h-10 flex items-center justify-center rounded-full bg-[#FFF8E2] ring-1 ring-yellow-200'
            )}
          >
            {money}
          </div>
        ...
        <div className="rounded-[6px] border border-gray-4 w-fit min-w-[369px] text-resin-black py-6 px-4 flex items-center justify-between">
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-10">
              <div className="space-y-1">
                <p className="text-[18px] leading-none font-semibold">
                  {active}
                </p>
                <span className="block text-[12px] text-gray-500">Active</span>
              </div>
              ...
              <div className="space-y-1">
                ...
                <span className="block text-[12px] text-gray-500">Repaid</span>
              </div>
              ...
              <div className="space-y-1">
                ...
                <span className="block text-[12px] text-gray-500">Overdue</span>
              </div>
            </div>
          </div>
      ...
      <Tabs defaultValue="all" className="w-full space-y-5 ">
        <TabsList className="border-b border-b-gray-200 w-full rounded-none">
          <TabsTrigger className="p-4 cursor-pointer" value="all">
            All Loans
          </TabsTrigger>
          <TabsTrigger className="p-4 cursor-pointer" value="active">
            Active Loans
          </TabsTrigger>
          <TabsTrigger className="p-4 cursor-pointer" value="repaid">
            Repaid Loans
          </TabsTrigger>
          <TabsTrigger className="p-4 cursor-pointer" value="overdue">
            Overdue Loans
          </TabsTrigger>
        </TabsList>
