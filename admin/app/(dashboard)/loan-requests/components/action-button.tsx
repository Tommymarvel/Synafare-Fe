...
import DeclineRequestModel from '../[id]/components/modals/decline-request';
import { useState } from 'react';

export type ActionType = 'offer' | 'rejected';
...
const ActionButton = ({
  ...
  id: string;
  handleAction: (action: ActionType) => void | Promise<boolean>;
  disabled?: boolean;
}) => {
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent className="bg-white py-0">
          <DropdownMenuItem className="border-b border-b-border-gray py-3 px-5 rounded-none">
            <a href={'/loan-requests/' + id}>View Request</a>
          </DropdownMenuItem>
          <DropdownMenuItem
