...
import type { Loan } from '../loans/types'; // adjust import if needed
...
const LoanRequest = () => {
  ...
  const baseNewRequests = loans.filter(
    (l) => l.loanStatus === STATUSCONST.PENDING
  );
  const baseLoanOffers = loans.filter(
    (l) => l.loanStatus === STATUSCONST.OFFER_RECEIVED
  );
  const baseDeclined = loans.filter(
  ...
  const loanAcceptedCount = baseLoanOffers.length;
  const loanDeclinedCount = baseDeclined.length;
  ...
  return (
    <div className="space-y-5">
      ...
      <Tabs defaultValue="requests" className="w-full space-y-5">
        <TabsList className="border-b border-b-gray-200 w-full rounded-none">
          <TabsTrigger className="p-4 cursor-pointer" value="requests">
            New Requests
          </TabsTrigger>
          <TabsTrigger className="p-4 cursor-pointer" value="offers">
            Loan Offers
          </TabsTrigger>
          <TabsTrigger className="p-4 cursor-pointer" value="declined">
            Declined Requests
          </TabsTrigger>
        </TabsList>

        {/* New Requests: show Status filter */}
